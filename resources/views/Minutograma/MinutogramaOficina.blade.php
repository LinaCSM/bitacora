...
<?php use App\Http\Controllers\MinutogramaController;
 use App\Http\Controllers\ProcesoSLAController;?>
<?php $tipoUsuario=Auth::user()->FK_Tipo?>
@section('Titulo')
    Minutograma mensual | Getronics
@stop
@section('contenido')
@if($tipoUsuario=="5" || $tipoUsuario=="6")

    <legend>Entregables Diarios <script>
            var date= new Date();
            var d= date.getDate();
            var day = (d<10) ? '0' + d:d;
    ...
    <div id="tablaMensuales" class="panel-body table-responsive" style="margin-bottom: 30px">
        <table class="table table-striped table-bordered table-hover table-responsive" id="tablaEntregablesMensuales">
            <thead>
            <tr>
                <th>Grupo País</th>
                <th>Job</th>
            ...
            <tbody>
                ...
                @foreach($procesosOficina as $Proceso)
